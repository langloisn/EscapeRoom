In order to compile and run our program, simply download the zip file and unzip it so that it is a folder.  Once you have done that, it is very important that the images are in the same folder as the have files in order to allow our program to use the images. With everything in the same folder, use the terminal to navigate to the title of the folder which is escapeRoom.  Once you are there, use javac *.java & java Main to run the program. 
